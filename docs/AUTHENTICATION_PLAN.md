...
interface User {
  ...
  createdAt: string;
  lastLoginAt?: string;
  lastActivityAt?: string; // Для проверки таймаута
}

// Данные для регистрации (с паролем)
...
- `registerUser(email, password, name)` - регистрация нового пользователя (с хешированием пароля)
- `loginUser(email, password)` - вход пользователя (с проверкой пароля и записью в историю)
...
- `hashPassword(password)` - хеширование пароля (Utilities.computeDigest)
- `verifyPassword(password, passwordHash)` - проверка пароля
- `addLoginHistory(email, success, failureReason)` - добавление записи в историю входов
...
- Лист "Пользователи" с колонками:
  ...
  - Email (уникальный)
  - Имя
  - Пароль (хеш пароля, НЕ открытый пароль)
  - Роль (admin/user)
  - Дата создания
  - Последний вход
...
- `POST /login` - вход (email, password, с записью в историю)
- `POST /logout` - выход
- `POST /change-password` - смена пароля (currentPassword, newPassword)
...
**Функция `registerUser(email, password, name)`:**
- Проверка уникальности email
- Валидация пароля (минимальная длина, сложность)
- Хеширование пароля (использовать Utilities.computeDigest или аналогичный метод)
- Создание записи пользователя с ролью 'user' по умолчанию
- Генерация UUID для ID
- Сохранение хеша пароля (НЕ сохранять пароль в открытом виде)
...
- Проверка пароля (сравнение хеша пароля с сохраненным хешем)
- Если пароль неверный - запись в историю входов (неуспешный вход) и возврат ошибки
...
**Функциональность:**
- Форма с полями: Email, Пароль, Подтверждение пароля, Имя (опционально)
...
- Показ/скрытие пароля (иконка глаза)
- Индикатор сложности пароля (опционально)
...
### Аутентификация
- **Email + пароль** - полная система аутентификации с паролями
- **Хеширование паролей** - пароли хранятся в виде хешей (использовать Utilities.computeDigest в Google Apps Script)
- **Таймаут сессии: 1 час** - если пользователь бездействует 1 час, требуется повторный вход
- **Смена пароля** - будет реализована (функция changePassword)
- **История входов** - необходима, сохранять дату и время каждого входа (успешные и неуспешные)
