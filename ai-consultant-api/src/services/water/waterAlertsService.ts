/**
 * waterAlertsService.ts
 *
 * Сводка проактивных алертов по воде: качество + поверка счётчиков.
 *
 * Структура:
 * 1. fetchWaterAlertsSummary — единый список для API и AI-инструмента
 */
...
export interface WaterAlertItem {
    id: string;
...
export interface WaterAlertsSummary {
    total: number;
    critical: number;
    warnings: number;
    items: WaterAlertItem[];
}

export async function fetchWaterAlertsSummary(): Promise<WaterAlertsSummary> {
    const items: WaterAlertItem[] = [];
    ...
    const { data: waterAlerts, error: waterError } = await supabase
        ...
        .limit(20);
    if (waterError) {
        console.warn('[WaterAlerts] water_quality_alerts error:', waterError.message);
    }
    ...
    if (metersError) {
        console.warn('[WaterAlerts] beliot_device_overrides error:', metersError.message);
    }
