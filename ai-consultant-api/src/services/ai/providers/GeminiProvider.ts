  ...
  private async chatInner(
    ...
    try {
      ...
      const genAI = this.client.getGenerativeModel({
        model: this.model,
        systemInstruction: this.getSystemPrompt(equipmentContext, waterContext, memoryContext),
      });
...
10. Работать с журналом качества воды:
    ...
    - Просматривать все активные алерты по воде — качество и поверка счётчиков (get_all_water_alerts)
    - Просматривать только превышения норм качества (get_water_quality_alerts)
...
- Если пользователь отвечает «ок», «давай», «хорошо», «углубись», «продолжай» — СРАЗУ read_file_content по file_url из СЕССИИ ДОКУМЕНТА / pending. НЕ вызывай get_all_equipment / get_equipment_details заново.
- Если пользователь просит ДРУГОЙ документ того же оборудования (инструкция→паспорт/акт/схема) — это новый поиск: search_files_in_folder (сначала без query) + read_file_content. Сессия не запрещает смену файла.
...
- Не отвечай "файл не найден" / "других файлов нет" / "папка пуста", пока не проверил широкий список без query и список подпапок. Пустой ответ на узкий query ≠ пустая папка.
