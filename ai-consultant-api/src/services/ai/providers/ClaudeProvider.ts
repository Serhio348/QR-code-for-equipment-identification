...
Для детального анализа, сравнения или другого периода — вызывай инструменты get_water_readings / analyze_water_consumption / calculate_water_consumption.`
      : '';
...
- РАСХОД (calculate_water_consumption) — сколько воды израсходовано за период = end − start, где start = baseline (последнее ДО периода, если есть) иначе первое В периоде; end = последнее В периоде. Например: "45 м³ за февраль".
- НИКОГДА не суммируй показания счётчика — это бессмысленно. Расход всегда считается как разница показаний.
- Если пользователь спрашивает "сколько кубов", "расход за период", "потребление за месяц/день" — ВСЕГДА вызывай calculate_water_consumption и показывай формулу (end − start). Не считай расход "в голове".
- Если пользователь называет объект или место — сначала вызови get_water_devices чтобы найти device_id.
