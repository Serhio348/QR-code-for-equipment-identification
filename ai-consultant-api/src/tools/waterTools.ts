...
export const waterTools: Anthropic.Tool[] = [
    ...
    // ----------------------------------------
    // Tool 2b: Детерминированный расчёт расхода
    // ----------------------------------------
    {
        name: 'calculate_water_consumption',
        description: 'Детерминированно посчитать РАСХОД (сколько м³ за период) по данным Supabase: стартовое показание = baseline (последнее ДО начала периода, если есть) иначе первое В периоде; конечное = последнее В периоде; расход = end − start. Используй ВСЕГДА для вопросов "сколько кубов/расход за период/за день/за месяц", чтобы не путаться и не считать "в голове".',
        input_schema: {
            type: 'object' as const,
            properties: {
                device_id: {
                    type: 'string',
                    description: 'ID устройства/счётчика.',
                },
                device_name: {
                    type: 'string',
                    description: 'Название счётчика или объекта установки (альтернатива device_id). Агент автоматически найдёт нужный device_id.',
                ...
                period: {
                    type: 'string',
                    enum: ['day', 'week', 'month', 'year'],
                    description: 'Период (если date_from/date_to не заданы): day/week/month/year.',
                },
                date_from: {
                    type: 'string',
                    description: 'Дата начала в формате YYYY-MM-DD (переопределяет period).',
...
export async function executeWaterTool(
    ...
    switch (name) {
        ...
        // ----------------------------------------
        // Детерминированный расход: end − start
        // ----------------------------------------
        case 'calculate_water_consumption': {
            // Период
            let dateFrom: string;
            let dateTo: string;
            ...
            const targetDeviceId =
                (input.device_id as string)
                || (resolvedDeviceIds.length === 1 ? resolvedDeviceIds[0] : '');

            if (!targetDeviceId) {
                return {
                    period: { from: dateFrom, to: dateTo },
                    count: 0,
                    message: 'Укажите device_id (или device_name, который однозначно находит один счётчик).',
                    ...(resolvedMatches.length > 0 && { resolved_devices: resolvedMatches }),
                };
            }

            // baseline: последнее ДО периода
            const baselineRes = await supabase
                ...
                .limit(1)
                .maybeSingle();
            if (baselineRes.error) throw new Error(`Ошибка baseline: ${baselineRes.error.message}`);
            ...
            const firstRes = await supabase
                ...
                .limit(1)
                .maybeSingle();
            if (firstRes.error) throw new Error(`Ошибка first: ${firstRes.error.message}`);
            ...
            const lastRes = await supabase
                ...
                .limit(1)
                .maybeSingle();
            if (lastRes.error) throw new Error(`Ошибка last: ${lastRes.error.message}`);
            ...
            const startSource = baseline ? 'baseline' : 'in_period';
            const startDate = baseline?.reading_date ?? first.reading_date;
            const startValue = Number(baseline?.reading_value ?? first.reading_value);

            const endDate = last.reading_date;
            const endValue = Number(last.reading_value);

            const consumption = endValue - startValue;
            const unit = (last.unit || first.unit || baseline?.unit || 'м³') as string;
            ...
            return {
                ...
                start: {
                    ...
                    reading_value: roundNumber(startValue, 6),
                },
                end: {
                    source: 'in_period',
                    reading_date: endDate,
                    reading_value: roundNumber(endValue, 6),
                },
                unit,
                consumption_m3: roundNumber(consumption, 6),
                formula: 'расход = end − start',
                note: startSource === 'baseline'
                    ? 'start взят как baseline (последнее показание ДО начала периода) для точного расхода с первой секунды периода.'
                    : 'baseline до начала периода не найден, start взят как первое показание В периоде.',
            };
        }
