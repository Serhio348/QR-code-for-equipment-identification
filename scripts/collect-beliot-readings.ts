  ...
  try {
    ...
    for (const device of devices) {
      ...
      try {
        ...
        // НО: если мы в начале часа (0-44 минуты), это может быть задержка запуска → используем предыдущий час
        if (isApiDateValid && hoursDiff >= -0.5 && hoursDiff < 2) {
          // Если мы в начале часа (0-44 минуты), это задержка запуска → используем предыдущий час
          if (currentMinute < 45) {
            // Задержка запуска: скрипт должен был запуститься в предыдущем часе
            ...
            // Нормальный запуск: используем время из API
        ...
        // Приоритет 3: Если опрос в 0-44 минуты часа и API невалидно или устарело
        // Используем текущий час, чтобы не перезаписывать корректные данные за предыдущий час устаревшими значениями
        else {
          // Мы в начале часа (0-44 минуты) и API невалидно или устарело
          // Используем текущий час
          hourStart = new Date(now);
          hourStart.setMinutes(0, 0, 0);
          hourStart.setSeconds(0, 0);
          hourStart.setMilliseconds(0);
