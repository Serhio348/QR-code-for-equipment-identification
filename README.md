...
### AI-консультант (Multi-Provider: Claude + Gemini + DeepSeek)
- 🤖 **Умный помощник** - AI-консультант для работы с оборудованием и мониторингом воды
- 🔄 **Multi-Provider архитектура** - поддержка Claude, Gemini и DeepSeek с автоматическим fallback
...
- 🧠 **Долговременная память агента** - агент помнит факты между сессиями (тарифы, контакты, адреса)
  - Автоматическое обновление тарифов из новых счетов
  - Инструменты: `save_memory`, `get_memory`, `delete_memory`
- 🕐 **Фоновая история диалогов** - агент помнит предыдущий разговор, чат всегда начинается пустым
...
- 🔔 **Уведомления** - автоматические уведомления при появлении нового счёта, росте потребления >20%, изменении тарифа
...
### Push-уведомления
- 🔔 **Web Push на мобильных** - нативные push-уведомления на телефоне через сервис-воркер
- 💬 **Toast на десктопе** - всплывающие уведомления при открытии вкладки Вода
- 📋 **Типы уведомлений:**
  - Новый счёт водоканала (сумма, объём)
  - Потребление воды выросло >20% к прошлому месяцу
  - Изменился тариф на воду или канализацию
- 👥 **Несколько получателей** - уведомления можно настроить для нескольких пользователей
...
GAS_API_URL=https://script.google.com/macros/s/AKfycbyzLcAUNQE597JcAhgN-wQYo5CybIPxswBz1sgCmOM08qHljUQA4EERHqNmyrFjUxNGNQ/exec
...
3. Запустите миграции из `supabase/migrations/` в SQL Editor (по порядку):
   ...
   - `20260317_notifications.sql` — уведомления и Web Push подписки
4. Создайте Storage bucket `invoices` (Storage → New bucket → Name: invoices, Public: false)
5. Настройте Row Level Security (RLS) политики
6. Скопируйте URL и Anon Key в `.env.local`
...
- `INVOICE_SYNC_SECRET` - та же строка что в переменной `INVOICE_SYNC_SECRET` на Railway

**Первичный импорт всех счетов** (один раз после настройки):
```bash
curl -X POST https://your-ai-api.railway.app/api/invoices/sync-all \
  -H "X-Sync-Secret: ваш_секрет"
```

**Запустить вручную:** GitHub → Actions → Sync bvod.by Invoices → Run workflow
...
2. Настройте переменные окружения:
   ...
   - `VITE_VAPID_PUBLIC_KEY` - публичный VAPID ключ (тот же что в бэкенде)
...
3. **Настройте переменные окружения**:
   ...
   **Портал bvod.by (синхронизация счетов):**
   ```env
   BVOD_LOGIN=ваш_логин
   BVOD_PASSWORD=ваш_пароль
   INVOICE_SYNC_SECRET=случайная_строка  # защита эндпоинта /api/invoices/sync
   ```

   **Web Push уведомления:**
   ```env
   VAPID_PUBLIC_KEY=...   # сгенерировать: npx web-push generate-vapid-keys
   VAPID_PRIVATE_KEY=...
   VAPID_EMAIL=ваш@email.com
   NOTIFICATION_USER_IDS=uuid1,uuid2  # UUID пользователей из Supabase Auth → Users
   ```
